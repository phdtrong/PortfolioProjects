...
def split_dataset(X, node_indices, feature):
    ...
    ### START CODE HERE ###
    for element in node_indices:
        if (X[element,feature] == 1): left_indices.append(element)
        else: right_indices.append(element)
...
def compute_information_gain(X, y, node_indices, feature):
    ...
    information_gain = Hp - w_left*Hp_left - w_right*Hp_right
    ### END CODE HERE ###  
...
def get_best_split(X, y, node_indices):   
    ...
    for feature in range(num_features):        
        if (max_value < compute_information_gain(X,y,node_indices,feature)): 
            max_value = compute_information_gain(X,y,node_indices,feature)
