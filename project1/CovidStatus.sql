...
select location, population, max(total_cases), max(total_cases/population)*100 as cases_over_population_rates
from CovidDeaths$
group by location, population
order by 4 desc

--3.4. Showing countries with Highest Death Count per Population
select location, population, max(total_cases), max(total_deaths/population)*100 as deaths_over_population_rates
from CovidDeaths$
group by location, population
order by 4 desc
